...
import de.pauhull.bansystem.common.util.HashGenerator;
...
public class MuteTable {
    ...
    public MuteTable(MySQL mySQL, ExecutorService executorService) {
        ...
        mySQL.update("CREATE TABLE IF NOT EXISTS `" + TABLE + "` (`id` INT AUTO_INCREMENT, `uuid` VARCHAR(255), " +
                "`mute_id` VARCHAR(255), `time` BIGINT, `mutedby` VARCHAR(255), `secret_key` VARCHAR(255), PRIMARY KEY (`id`))");
    }
        ...
        executorService.execute(() -> {
            try {
                ...
                while (result.next()) {
                    ...
                    muteInfos.add(new MuteInfo(uuid, muteId, time, mutedBy));
                }
        ...
        executorService.execute(() -> {
            try {
                ...
                if (result.next()) {
                    ...
                    String muteId = result.getString("mute_id");
                    UUID mutedBy = UUID.fromString(result.getString("mutedby"));
                    consumer.accept(new MuteInfo(uuid, muteId, time, mutedBy));
                    return;
                }
    ...
    public void mute(UUID uuid, UUID mutedBy, long time, BiConsumer<String, String> consumer) {
        getMute(uuid, mute -> {
            generateFreeMuteIdAndSecretKey((muteId, secretKey) -> {
                if (mute != null) {
                    String sql = "UPDATE `" + TABLE + "` SET `mute_id`='" + muteId
                            + "', `time`=" + time + ", `mutedby`='" + mutedBy + "', `secret_key`='" + HashGenerator.generateHexMd5Hash(secretKey) + "' WHERE `uuid`='" + uuid.toString() + "'";
                    mySQL.update(sql);
                } else {
                    String sql = "INSERT INTO `" + TABLE + "` VALUES (0, '" + uuid + "', '" + muteId + "', "
                            + time + ", '" + mutedBy + "', '" + HashGenerator.generateHexMd5Hash(secretKey) + "')";
                    mySQL.update(sql);
                }
                consumer.accept(muteId, secretKey);
            });
        });
    }
    ...
    public void generateFreeMuteIdAndSecretKey(BiConsumer<String, String> consumer) {
        executorService.execute(() -> {
            ...
            do {
                ...
                String md5 = HashGenerator.generateHexMd5Hash(builder.toString());

                try {
                    ResultSet result = mySQL.query("SELECT * FROM `" + TABLE + "` WHERE `secret_key`='" + md5 + "'");
                    if (!result.isBeforeFirst()) {

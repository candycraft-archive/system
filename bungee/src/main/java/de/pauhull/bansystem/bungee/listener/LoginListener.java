...
import de.pauhull.bansystem.bungee.util.PermissionHelper;
...
public class LoginListener implements Listener {
    ...
    public void onLogin(LoginEvent event) {
        ...
        if (plugin.isMaintenance()) {
            try {
                List<String> permissions = PermissionHelper.getPermissions(uuid);
                if (!permissions.contains("*") && !permissions.contains(plugin.getMaintenancePermission())) {
                    event.setCancelReason(plugin.getMaintenanceMessage());
